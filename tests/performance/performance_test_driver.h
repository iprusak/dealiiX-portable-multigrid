...
struct Measurement
{
  Measurement(std::initializer_list<double> results)
    : timing(results)
  {}
  Measurement(std::vector<double> &results)
    : timing(results)
  {}
...
main(int argc, char *argv[])
{
  ...
  // AssertThrow(metric != Metric::instruction_count || number_of_repetitions ==
  // 1,
  //             dealii::ExcMessage(
  //               "Test specified an invalid number of measurements."));


  for (unsigned int refinement_cycles = 0; refinement_cycles < 30;
       ++refinement_cycles)
    {
      pout << "Refinement cycles: " << refinement_cycles << std::endl
           << std::endl;
      ...
      AssertThrow(metric != Metric::instruction_count ||
      ...
      std::generate_n(std::back_inserter(measurements),
                      ...
                      [&]() {
                        return perform_single_measurement(refinement_cycles);
                      });
      ...
      if (number_of_repetitions == 1)
      ...
      else
        {
          ...
          pout << std::left << std::setw(22) << "name" << std::right
               << std::setw(11) << "min" << std::setw(11) << "max"
               << std::setw(11) << "mean" << std::setw(11) << "std_dev"
               << std::setw(22) << "mean / solver " << std::setw(9) << "samples"
          ...
          switch (metric)
            {
              case Metric::timing:
                ...
                //     // pout << names[i] << "\t" << x << "\n";
                //     pout << std::left << std::setw(30) << names[i] << x <<
                //     "\n";
                //   }
                ...
                if (number_of_repetitions == 1)
                  {
                    pout << std::left << std::setw(22) << "name" << std::right
                         << std::setw(11) << "min" << std::setw(11) << "max"
                         << std::setw(11) << "mean" << std::setw(11)
                         << "std_dev" << std::setw(9) << "samples" << std::endl;
                  }
                else
                  {
                    // pout << std::left << std::setw(30) << "name" <<
                    // std::right
                    //      << std::setw(12) << "min" << std::setw(12) << "max"
                    //      << std::setw(12) << "mean" << std::setw(12)
                    //      << "std_dev" << std::setw(10) << "samples"
                    //      << std::endl;
                    ...
                    const double quantity_per_solver =
                      mean / measurements[0].timing[9] *
                      100; // assuming solve2 is at index 9;

                    // pout << names[i] << "\t" << min << "\t" << max << "\t" <<
                    // mean
                    //      << "\t" << std_dev << "\t" << number_of_repetitions
                    //      << "\n";
                    // pout << std::left << std::setw(25) << names[i] << "\t"
                    //      << min << "\t" << max << "\t" << mean << "\t"
                    //      << std_dev << "\t" << number_of_repetitions << "\n";

                    pout << std::left << std::setw(22) << names[i] << std::right
                         << std::fixed << std::setprecision(7) << std::setw(11)
                         << min << std::setw(11) << max << std::setw(11) << mean
                         << std::setw(11) << std_dev << std::setw(22)
                         <<std::fixed << std::setprecision(2)
                         << quantity_per_solver << "%" << std::setw(9)
                         << number_of_repetitions << "\n";
                  }
                break;
